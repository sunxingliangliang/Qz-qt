...
uniteaxios.install = function (Vue) {
  ...
  axios.interceptors.response.use(function (resp) {
    ...
    if (apiToken) {
        Store.commit(types.UPDATE_API_TOKEN, apiToken)
    }
    if (resp.data.code == constants.REQUEST_API_TOKEN_INVALID) {       // 会话超时
        ...
        if(resp.data.message){
            Vue.$vux.toast.show({text: resp.data.message, type: 'warn', position: 'middle'})
        }
    ...
    Vue.$vux.toast.show({text: message, type: 'warn', position: 'middle'})
