...
export default {
  ...
  methods: {
    ...
    load () {
      ...
      setTimeout(() => {
        this.count += 6
        this.$http.get('pc/platform/merchantPlatfrom?status= ').then(res => {
          var { code, data } = res.data
          if (code === 1000) {
    ...
    load1 () {
      ...
      setTimeout(() => {
        this.count += 6
        this.$http.get('pc/platform/merchantPlatfrom?status=1').then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.list = data.content
            this.list.forEach(item => {
    ...
    load2 () {
      ...
      setTimeout(() => {
        this.count += 6
        this.$http.get('pc/platform/merchantPlatfrom?status=2').then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.list = data.content
            this.list.forEach(item => {
