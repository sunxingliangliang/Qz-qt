        ...
        <el-tab-pane label="修改密码" name="second">
          <div :class="$style.f_userxx">
            ...
            <div :class="$style.f_userxinxi1">
              <span>原密码</span>
              <el-input placeholder="请输入内容" :class="$style.f_inpt3" v-model="password" show-password clearable></el-input>
            </div>
            <div :class="$style.f_userxinxi1">
              <span>新密码</span>
              <el-input placeholder="请输入内容" :class="$style.f_inpt3" v-model="newpassword" show-password clearable></el-input>
            </div>
            <div :class="$style.f_userxinxi1">
              <span>确认密码</span>
              <el-input
          ...
          <div :class="$style.clear"></div>
          <div :class="$style.f_bc">
            <el-button :class="$style.f_btn" @click="keep">保存</el-button>
          </div>
        </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    user () {
      ...
      this.input6 = userinfo.user.merchantInfo.name
      this.input7 = userinfo.user.merchantInfo.phone
      this.input8 = userinfo.user.merchantInfo.email
      this.input9 = userinfo.user.merchantInfo.address
      let province = ''
      this.getSearch().then(res => {
        ...
        this.getSearch(userinfo.merchant.province).then(res => {
          res.forEach(item => {
            if (userinfo.merchant.city === item.id) {
              this.input5 = province + '\\' + item.name
            }
          })
        })
      })
    ...
    keep () {
      let info = {
        ...
        newPassword: this.affirmpassword
      }
      this.$http.put(`sys/user/update_password`, info).then(res => {
