    ...
    <div :class="$style.f_cjdd">
      ...
      <div :class="$style.f_bgnr">
        <el-table :data="tableData2" border style="width: 100%;">
          ...
          <el-table-column prop="type" align="center" label="客户类型"></el-table-column>
        ...
        <el-table :data="tableData4" border style="width: 100%;">
          ...
          <el-table-column prop="scencesAddress" align="center" label="场景类型">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.type==='2'">{{scope.row.industryTypeParent}}/{{scope.row.industryParent}}</span>
            </template>
          </el-table-column>
  ...
  methods: {
    ...
    getGrouping () {
      this.$http.get(`pc/task/detail/${this.id}`).then(res => {
        ...
        if (code === 1000) {
          ...
          console.log(this.row.deviceList)
    ...
    terminal () {
      if (this.selectedOptions.length === 1) {
        ...
        this.$http.get(`pc/group/findDevicesByGroup?groupId=${this.selectedOptions[0][0]}`,{params:{
    ...
    adjunction () {
      this.$jsonp(`http://api.map.baidu.com/geocoder/v2/?ak=1IGwblSXzAV0yxzCq0ZGdYoixoreCQwS&callback=renderReverse&location=${this.lat},${this.lng}&output=json&pois=1 `).then(res => {
        ...
        //   deviceids:
        // }
        // this.$http.post(`task/updateDevice`)
      }).catch((err) => {
        console.log('错误信息' + err)
      })
    },
