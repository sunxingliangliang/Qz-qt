  ...
  methods: {
    ...
    gather2 () {
      if (this.trade.length === 2) {
        ...
        this.$http.get(`modules/signIndustryType/listByParent?pid=${this.trade[0]}`, {          params: {
            pid:this.trade[0],
          }        }).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
    ...
    cli3 () {
      ...
      if (this.onedistrict.length === 2) {
        ...
        this.$http.get(`modules/signRegion/listByParent?pid=${this.onedistrict[1]}`, {          params: {
            pid:this.onedistrict[0],
          }        }).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            console.log(data)
