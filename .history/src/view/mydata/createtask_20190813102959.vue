    ...
    adjunction () {
      ...
      if (this.repeat === '重复') {
        ...
        this.Groupingend.forEach(item => {
          lat = item.lat
          lng = item.lng
          this.$jsonp(`http://api.map.baidu.com/geocoder/v2/?ak=1IGwblSXzAV0yxzCq0ZGdYoixoreCQwS&callback=renderReverse&location=${lat},${this.lng}&output=json&pois=1 `).then(res => {
            // var {code,data}=res.data
