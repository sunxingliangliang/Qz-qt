  ...
  methods: {
    ...
    submit () {
      this.$confirm('是否确定发布任务, 是否继续?', '提示', {
        ...
        info.append('price', this.missionname)
        info.append('amountCount', this.missionname)
        info.append('fromTime', this.missionname)
        info.append('toTime', this.missionname)
        info.append('remarks', this.missionname)
        info.append('beans', this.missionname)
        info.append('file', this.missionname)
        // console.log(this.url.substring(5))
        this.$http.post(`pc/platform/addPlatformTask`,info,{headers:{"Content-Type": "multipart/form-data"}}, ).then(res => {
