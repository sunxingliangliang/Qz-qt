    ...
    return {
      ...
      props: {
        ...
        lazyLoad (node, resolve) {
          ...
          // setTimeout(() => {
          let option = {
            url: `modules/area/areaByPid?pid=1`,
            methods: 'get',
            data: {
              groupId: level
            }
          ...
          if (level === 0) {
            option.url = 'modules/area/areaByPid?pid=1'
            console.log(node)
          } else if (level === 1) {
      ...
      props1: {
        ...
        lazyLoad (node, resolve) {
          ...
          if (level === 0) {
            ...
            option.url = `modules/signIndustry/listByParent?pid=${node.value}`
          }
          axios(option).then(res => {
            let { meta, data, code } = res.data
            if (code === 1000) {
      ...
      props4: {
        ...
        lazyLoad (node, resolve) {
          ...
          if (level === 0) {
            ...
            option.url = `modules/signRegion/listByParent?pid=${node.value}`
          }
          axios(option).then(res => {
            let { meta, data, code } = res.data
            if (code === 1000) {
      ...
      props2: {
        ...
        lazyLoad (node, resolve) {
          ...
          if (level === 0) {
            option.url = `modules/signRegion/listByParent?pid=${this.onedistrict[1]}`
          } else if (level === 1) {
            option.url = `modules/signRegion/listByParent?pid=${node.value}`
          }
          axios(option).then(res => {
  ...
  methods: {
    ...
    gatherplace () {
      ...
      if (this.adress.length === 3) {
        ...
        this.$http.get(`modules/area/areaByPid?pid=${this.adress[0]}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            data.forEach(item => {
        ...
        this.$http.get(`modules/area/areaByPid?pid=${this.adress[1]}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            // console.log(data)
            data.forEach(item => {
    ...
    cli2 () {
      if (this.trade.length === 2) {
        ...
        this.$http.get(`modules/signIndustry/listByParent?pid=${this.trade[0]}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            console.log(data)
            data.forEach(item => {
    ...
    gather2 () {
      if (this.trade.length === 2) {
        ...
        this.$http.get(`modules/signIndustryType/listByParent?pid=${this.trade[0]}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            console.log(data)
            data.forEach(item => {
    ...
    cli3 () {
      ...
      if (this.onedistrict.length === 2) {
        ...
        this.$http.get(`modules/signRegion/listByParent?pid=${this.onedistrict[1]}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            console.log(data)
    ...
    areaname () {
      ...
      if (this.twodistrict.length === 2) {
        ...
        this.$http.get(`modules/signRegion/listByParent?pid=${this.twodistrict[0]}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            data.forEach(item => {
    ...
    submit () {
      this.$confirm('是否确定发布任务, 是否继续?', '提示', {
        ...
        this.$http.post(`pc/platform/addPlatformTask`, qs.stringify(info, { arrayFormat: 'indices', allowDots: true })).then(res => {
