  ...
  <div :class="$style.f_bj">
    ...
    <div :class="$style.f_cjdd">
      ...
      <el-row :class="$style.f_row">
        ...
        <el-input clearable :class="$style.f_inpt" v-model="datanum" @blur="planning" placeholder="请选择"></el-input>
        <p :class="$style.f_rwmc">单条奖励</p>
        <el-input clearable :class="$style.f_inpt" v-model="single" @blur="planning" placeholder="请选择"></el-input>
...
export default {
  ...
  methods: {
    ...
    planning(){
      console.log(1)
      this.input1=  this.datanum *this.single
    },
    // 地理围栏
    Geofence () {
      this.Fence = true
    ...
    submit () {
      this.$confirm('是否确定发布任务, 是否继续?', '提示', {
        ...
        info.append('TaskCollectType', this.datatype)
        info.append('radius', this.distance)
        info.append('fromDate', this.collectiondate[0])
        info.append('toDate', this.collectiondate[1])
        info.append('price', this.datanum)
        info.append('amountCount', this.single)
        info.append('fromTime', starttime)
        info.append('toTime', endtiem)
        info.append('remarks', this.textarea2)
        info.append('file', this.url1.raw)
        // this.beanse.forEach(item=>{
        // info.append('beans.scencesBaseId', item.scencesBaseId)
        // info.append('beans.crowdId', item.crowdId)
        // })
       for(let i  in this.beanse){
        info.append(`beans.scencesBaseId`+[i], this.beanse[i].scencesBaseId)
        info.append(`beans.crowdId[i]`+[i], this.beanse[i].crowdId)
       }
        // console.log(this.url.substring(5))
