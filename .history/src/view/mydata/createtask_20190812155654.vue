...
export default {
  ...
  methods: {
    ...
    sceneheading () {
      ...
      if (this.datastamp === 1) {
        // 区域
        this.$http.get(`pc/collect/getScencesName?province=${this.adress[0]}&city=${this.adress[1]}&area=${this.adress[2]}&type=1&industryParent=${this.onedistrict[0]}&industry=${this.onedistrict[1]}&industrytypeParent=${this.twodistrict[0]}&industrytype=${this.twodistrict[1]}`,{par}).then(res => {
          var { code, data } = res.data
          console.log(data)
          if (code === 1000) {
            this.field = data
