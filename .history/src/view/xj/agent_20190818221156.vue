<template>
  <div :class="$style.f_fz">
    <div class="f_background">
      <div :class="$style.f_s" v-if="proxyType===3">
      ...
      <div :class="$style.f_s6" v-if="proxyType===1">
        <el-card shadow="always" :class="$style.f_kp" >
          ...
          <div :class="$style.f_sjvkt">
            <p :class="$style.numvkt">{{shi}}</p>
            <p :class="$style.f_sjfw">市级运营中心</p>
          </div>
        </el-card>
      </div>
      <div :class="$style.f_s6"  v-if="proxyType===1||proxyType===3">
        <el-card shadow="always" :class="$style.f_kp">
          <div :class="$style.f_icon">
            <i class="iconfont icondaili"></i>
          </div>
          <div :class="$style.f_sjvkt">
            <p :class="$style.numvkt">{{yiban}}</p>
  ...
  methods: {
    getCount () {
      this.$http.get(`pc/merchant/followerCount`).then(res => {
        ...
        if (code === 1000) {
          console.log(data)
          this.shi = data.市级运营中心
          this.yiban = data.市级一般代理商
          this.all1=  this.shi+this.yiban
          this.all3=this.yiban 
        }
      }).catch(err => {
