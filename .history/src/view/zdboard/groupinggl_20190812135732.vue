        ...
        >
          ...
          <div>
            <el-table
              ...
              @selection-change="handleselection-changeionChange"
            >
              <el-table-column prop="code" align="center" label="采集终端" show-overflow-tooltip></el-table-column>
              <el-table-column type="selection-changeion" align="center" label="操作"></el-table-column>
            </el-table>
            <el-table
              :data="Grouping2"
              tooltip-effect="dark"
              ...
              @selection-change="handleselection-changeionChange2"
            >
              <el-table-column prop="code" align="center" label="采集终端" show-overflow-tooltip></el-table-column>
              <el-table-column type="selection-changeion" align="center" label="操作"></el-table-column>
            </el-table>
            <el-table
              ...
              border
              style="width: 30%;margin-top:20px;display: inline-block;margin-left:15px;"
              @selection-change="handleselection-changeionChange3"
            >
              <el-table-column prop="code" align="center" label="采集终端" show-overflow-tooltip></el-table-column>
              <el-table-column type="selection-changeion" align="center" label="操作"></el-table-column>
            </el-table>
        ...
        >
          ...
          <div>
            <el-table
              ...
              style="width: 30%;margin-top:20px;display: inline-block;margin-left:20px;"
              @selection-change="handleselection-changeionChange4"
            >
              <el-table-column prop="code" label="采集终端" show-overflow-tooltip></el-table-column>
              <el-table-column type="selection-changeion" align="center"></el-table-column>
            </el-table>
            <el-table
              ref="multipleTable2"
              :data="tableData2"
              ...
              border
              style="width: 30%;margin-top:20px;display: inline-block;margin-left:15px;"
              @selection-change="handleselection-changeionChange5"
            >
              <el-table-column prop="code" align="center" label="采集终端" show-overflow-tooltip></el-table-column>
              <el-table-column type="selection-changeion" align="center" label="操作"></el-table-column>
            </el-table>
            <el-table
              ...
              border
              style="width: 30%;margin-top:20px;display: inline-block;margin-left:15px;"
              @selection-change="handleselection-changeionChange6"
            >
              <el-table-column prop="code" align="center" label="采集终端" show-overflow-tooltip></el-table-column>
              <el-table-column type="selection-changeion" align="center" label="操作"></el-table-column>
            </el-table>
...
export default {
  ...
  methods: {
      ...
      this.$http.get(`pc/group/all`).then(res => {
        ...
        if (code === 1000) {
          ...
          for (let i in this.tableData) {
            this.tableData[i].num = this.tableData[i].deviceList.length
            //  console.log(this.tableData[i].deviceList)
            this.tableData[i].deviceList.forEach(item => {
              name.push(item.code)
              this.tableData[i].code = name.join(',')
            })
          }
        }
      }).catch((err) => {
        ...
        for (let i in arr) {
          for (let a in this.tableData1) {
            if (arr[i].id === this.tableData1[a].id) {
              this.$refs.multipleTable.toggleRowselection-changeion(this.tableData1[0], true)
            }
          }
          for (let b in this.tableData2) {
            if (arr[i].id === this.tableData2[b].id) {
              this.$refs.multipleTable2.toggleRowselection-changeion(this.tableData2[b], true)
            }
          }
          for (let c in this.tableData3) {
            if (arr[i].id === this.tableData3[c].id) {
              this.$refs.multipleTable3.toggleRowselection-changeion(this.tableData3[c], true)
            }
          }
        }
    ...
    certain () {
      ...
      this.$http.put(`pc/group/updateGroup?name=${this.GroupName}&id=${this.Serialnumber}&ids=${ids}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.$message({
            message: '创建成功',
    ...
    determine () {
      console.log(this.addGrouping)
      console.log(this.addGrouping1)
      console.log(this.addGrouping2)
      let ids = this.addGrouping.concat(this.addGrouping1)
      this.ids = ids.concat(this.addGrouping2)
      console.log(this.ids)
      // this.$http.get(`pc/group/addGroup`, {        params: {
      //     name:this.GroupName,
      //   }      }).then(res => {
      //   var { code, data } = res.data
      //   if (code === 1000) {
    ...
    handleselection-changeionChange (val) {
      // console.log(val)
      this.addGrouping=[]
      val.forEach(item=>{
    ...
    handleselection-changeionChange1 (val) {
      // console.log(val)
      this.addGrouping1=[]
      val.forEach(item=>{
    ...
    handleselection-changeionChange2 (val) {
      this.addGrouping2=[]
      val.forEach(item=>{
        this.addGrouping2.push(item.id)
      })
    },
    handleselection-changeionChange3 (val) {
      console.log(val)
      this.ids.push(val.id)
    },
    handleselection-changeionChange4 (val, row) {
      console.log(val)
      console.log(row.id)
      this.ids.push(row.id)
    },
    handleselection-changeionChange5 (val) {
      console.log(val)
      this.ids.push(val.id)
    },
    handleselection-changeionChange6 (val) {
      console.log(val)
      this.ids.push(val.id)
    },
