...
export default {
  ...
  methods: {
    ...
    sjxq (index, row) {
      ...
      console.log(row)
      this.$router.push({ path: '/index/rwpoint.vue', query: { id: row.gtId } })
    },
    sjxq2 (index, row) {
      this.$store.commit('myval1', this.btname)
      console.log(index)
      // this.$router.push('/index/rwpoint.vue')
      this.$router.push({ path: '/index/datafiexpoint.vue', query: { id: row.taskId } })
    },
    queding () {
    ...
    tijiao () {
      ...
      this.$http.post('pc/device/binding', info).then(res => {
        ...
        if (code === 1000) {
          this.$message({
            type: 'success',
            message: '已成功添加' + data.success.length + "台,失败" + data.error.length + "台"
          });
    ...
    tijiao1 () {
      ...
      this.$http.post(`pc/device/untied`, info).then(res => {
        ...
        if (code === 1000) {
          ...
          this.jiebang = false
          this.alltableData.splice(this.index,1)
          this.total--
          //  rows.splice(index, 1);
          // this.qb()
        } else if (code == 2001) {
