  ...
  <div class="bj">
    ...
    <div :class="$style.f_table">
      <div :class="$style.f_row" style="padding-top:39px;">
        ...
        <div :span="1" style="margin-top: 5px;" :class="{'f_fy':xz2,'f_fy1':xz3}" @click="ywc">已领取</div>
        <div :span="1" style="margin-top: 5px;" :class="{'f_fy':xz4,'f_fy1':xz5}" @click="wwc">未领取</div>
      </div>
      <!-- 全部 -->
      <div v-show="xz1===true" class="infinite-list-wrapper">
        ...
        >
          <li v-for="(item,i) in list" :key="i" :class="$style.f_list">
            ...
            <div :class="$style.f_ysj" v-if="item.isreceive===0">
              <div :class="$style.f_yjxz">进行中</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    receive (item) {
      ...
      this.$router.push({ path: '/index/lqrenwu.vue', query: { row: item.id } })
    },
    received (item) {
      console.log(item)
      this.$store.commit('myval1', this.btmatter)
      this.$router.push({path:'/index/rwhuigou.vue',query:{id:item.id}})
    },
