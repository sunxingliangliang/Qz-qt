    ...
    <div :class="$style.f_sx">
      ...
      <el-row :class="$style.search">
        <div style="margin-left:20px;display: inline-block;">
          <span>数据类型</span>
          <el-select style="margin-left: 3px;" v-model="datastamp" clearable placeholder="请选择" >
            <el-option
  ...
  methods: {
    ...
    visit () {
      ...
      for (let i in this.oneself) {
        ...
        this.province.forEach(item => {
          if (id === item.id && !this.Provide.includes(id)) {
            // console.log(item)
            sheng = item.name
            this.Provide.push(sheng)
          }
        })
      }
    ...
    downtownarea () {
      ...
      for (let i in this.city) {
        let id = this.city[i]
        this.urbandistrict.forEach(item => {
          if (id === item.id && !this.City.includes(id)) {
            shi = item.name
            this.City.push(shi)
          }
        })
    ...
    district () {
      ...
      for (let i in this.area) {
        let id = this.area[i]
        this.countylevel.forEach(item => {
          if (id === item.id && !this.Area.includes(id)) {
            qu = item.name
            this.Area.push(qu)
          }
        })
