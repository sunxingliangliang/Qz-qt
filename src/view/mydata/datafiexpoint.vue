...
export default {
  ...
  methods: {
    getList () {
      this.$http.get(`pc/fixed/selectFixedById`, {        params: {
        ...
        if (code === 1000) {
          ...
          console.log(data.decice.deviceList)
          ...
          if (code === 1000) {
            ...
            data.forEach(item => {
              ...
              this.tableData.forEach(item1 => {
                if (this.terminalobj.code === item1.code) {
                  this.repeat = '重复'
                } else {
                  this.repeat = ''
                }
    ...
    adjunction () {
      ...
      this.$http.post(`pc/fixed/updateFixedDevice`, info).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.$message({
            message: '添加成功',
    ...
    handleDelete (index, rows,row) {
      ...
      this.$http.post(`pc/fixed/delFixedDevice`,info).then(res => {
        var { code, data } = res.data
        console.log(data)
        if (code === 1000) {
          this.$message({
