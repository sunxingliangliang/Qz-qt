...
export default {
  ...
  methods: {
    // 采集任务列表
    getList () {
      this.$http.get(`pc/task/list?tab=1&size=${10}`).then(data => {
    ...
    handleSizeChange (val) {
      this.sizes = val;
      this.$http.get(`pc/task/list?tab=1&size=${val}`).then(res => {
    ...
    handleCurrentChange (val) {
      this.$http.get(`pc/task/list?tab=1&page=${val}&size=${this.sizes}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
