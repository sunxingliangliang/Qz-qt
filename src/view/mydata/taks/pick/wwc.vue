...
export default {
  ...
  methods: {
    // 采集任务列表
    getList () {
      this.$http.get(`pc/task/list?tab=2&size=${10}`).then(data => {
    ...
    handleSizeChange (val) {
      this.sizes = val;
      this.$http.get(`pc/task/list?tab=2&size=${val}`).then(res => {
    ...
    handleCurrentChange (val) {
      this.$http.get(`pc/task/list?tab=2&page=${val}&size=${this.sizes}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
